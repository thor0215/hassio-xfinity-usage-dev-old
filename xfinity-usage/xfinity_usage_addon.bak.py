...
class xfinityUsage ():
    def __init__(self, playwright: Playwright) -> None:
        ...
        self.Authentication_Url = 'https://login.xfinity.com/login'
        self.Authentication_Url = 'https://www.xfinity.com/learn/internet-service/auth'
        self.Authentication_Session_Url = 'https://api.sc.xfinity.com/user'
        self.Authentication_Session_Url = 'https://api.sc.xfinity.com/session/ssm/user'
        ...
        #self.browser = playwright.chromium.launch(headless=False,channel='chrome',devtools=True)
        self.browser = playwright.chromium.launch(headless=True,channel='chrome')
        #self.browser = playwright.firefox.launch(headless=True)

        """
        if  SUPPORT and \
            os.path.exists('/config/'):
            #self.browser.start_tracing(path=f'/config/{datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%f")}-trace.json')
            self.browser.start_tracing(path=f'/config/trace.json',screenshots=True)
        """
        if  SUPPORT and \
            os.path.exists('/config/'):
            self.context = self.browser.new_context(
                ...
                record_har_path=f'/config/{datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%f")}-requests.har'
            )
        else:
            self.context = self.browser.new_context(
                service_workers="block",
                screen={"width": 1280, "height": 720},
                viewport={"width": 1280, "height": 720}, is_mobile=True
            )
    ...
    def abort_route(self, route: Route) :
        ...
        if  route.request.resource_type not in bad_resource_types and \
            any(fnmatch.fnmatch(urllib.parse.urlsplit(route.request.url).netloc, pattern) for pattern in good_domains) :
                #print(urllib.parse.urlsplit(route.request.url).netloc)
                route.continue_()
        else:
    ...
    def check_responses(self,response) -> None:
        ...
        if response.ok:
            ...
            if  response.url.startswith(self.Session_Url) and \
                ...
                if  int(response.json()['session']['time_left']) >= 60:
            ...
            if  response.url.startswith(self.Authentication_Session_Url) and \
                response.json()['authGuid'] != 'NULL':
                    self.is_session_active = True
    ...
    def autheticate_user(self) -> None:
        self.page.goto(self.Authentication_Url)
        #self.page.wait_for_url(f'{self.Authentication_Url}*',timeout=self.timeout)
        self.page.wait_for_load_state('networkidle')

        expect(self.page.locator("input#user")).to_be_visible()
        ...
        #self.page.wait_for_url(f'{self.Authentication_Url}*',timeout=self.timeout)
        self.page.wait_for_load_state('networkidle')
        expect(self.page.locator("input#passwd")).to_be_visible()
        ...
        while(self.is_session_active == False):
            """
            if (self.is_session_active == False and self.page.url.startswith(self.Authentication_Url)):
                try:
                    self.page.wait_for_url(f'{self.Login_Url}*',timeout=self.timeout)
                except:
                    logging.info(f"Loading Login Page failed (URL: {self.parse_url(self.page.url)})")
            """
                ...
                if self.page.url.startswith(self.Login_Url):
                    ...
                    if self.is_session_active and self.usage_data == None:
                        if self.reload_counter < 5:
                            ...
                            #self.page.goto(self.Internet_Service_Url)
                            #self.page.goto(urllib.parse.urlparse(self.page.url).geturl())
                            self.page.reload(timeout=self.timeout,wait_until="networkidle")
